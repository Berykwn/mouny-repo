...
export default function HealthAndTrendSection({
    ...
    const scoreColor =
        score >= 80 ? 'text-positive' :
            score >= 60 ? 'text-amber-500' :
                score >= 40 ? 'text-orange-500' : 'text-negative'

    const barColor =
        score >= 80 ? 'bg-positive' :
            score >= 60 ? 'bg-amber-500' :
                score >= 40 ? 'bg-orange-500' : 'bg-negative'
        ...
        <div className="card px-4 py-3.5 space-y-3">
            ...
            <div className="space-y-2">
                ...
                {!hasTrend ? (
                    ...
                    <div>
                        <div className="grid grid-cols-4 pb-1.5 border-b border-line-soft">
                            <p className="text-[10px] text-muted-ink">Period</p>
                            <p className="text-[10px] text-muted-ink text-right">Income</p>
                            <p className="text-[10px] text-muted-ink text-right">Expense</p>
                            <p className="text-[10px] text-muted-ink text-right">vs prev</p>
                        </div>
                            ...
                            return (
                                <div
                                    ...
                                    className={cn(
                                        ...
                                        i < trendPeriods.length - 1 && 'border-b border-dashed border-line-soft'
                                    )}
                                >
                                    <div className="flex items-center gap-1">
                                        ...
                                        <p className={cn(
                                            'text-[11px]',
                                            isActive ? 'text-ink font-medium' : 'text-muted-ink'
                                        )}>
                                            {p.label}
                                        </p>
                                    </div>
                                    <p className={cn(
                                        ...
                                        isActive ? 'text-positive font-medium' : 'text-muted-ink'
                                    )}>
                                    ...
                                    <p className={cn(
                                        'text-[11px] text-right',
                                        diffPct === null ? 'text-muted-ink' :
                                            diffPct <= 0 ? 'text-positive' : 'text-negative'
                                    )}>
            ...
            <div className="space-y-2">
                ...
                {reasons.length > 0 && (
                    <p className="text-[10px] text-muted-ink leading-relaxed truncate">
                        {reasons[0]}
                    </p>
                )}
            </div>
